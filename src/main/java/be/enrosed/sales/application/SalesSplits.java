...
public class SalesSplits {
    public record Choice(Long lineId, int laterQuantity) {}
    ...
    public SalesSplitPricing pricing(SalesOrder order) {
        ...
        SalesSplitPricing terms = read(part.pricingJson);
        BigDecimal extraPct = Money.nz(order.extraDiscountPct());
        BigDecimal extra = same(extraPct, terms.extraDiscountPercent()) ? terms.extraDiscountAmount()
                : Money.money(Money.percentOf(subtotal(terms).subtract(terms.orderDiscountAmount()), extraPct));
        return new SalesSplitPricing(terms.lines(), terms.orderTierPercent(), terms.orderDiscountAmount(), extraPct, extra,
                Money.money(order.manualFreightEur()), terms.handling(), terms.sourceMeetsMinimum(), terms.vatRatePct(), terms.vatTreatment(),
                same(extraPct, terms.extraDiscountPercent()) ? terms.goodsTotal() : subtotal(terms).subtract(terms.orderDiscountAmount()).subtract(extra));
    }
    ...
    public SalesOrder ready(long id) {
        ...
        order.lines().forEach(line -> needed.merge(line.productId(), line.quantity(), Math::addExact));
        Map<Long, Product> catalog = catalog();
    ...
    void requireUpdate(SalesOrder before, SalesOrder after) {
        ...
        boolean changed = !Objects.equals(before.customerId(), after.customerId())
                ...
                || !Objects.equals(before.pallets(), after.pallets());
    ...
    private Plan plan(SalesOrder source, Request request) {
        ...
        Map<Long, Integer> laterById = new HashMap<>();
        for (Choice choice : selection.lines()) if (laterById.put(choice.lineId(), choice.laterQuantity()) != null)
        ...
        for (int i = 0; i < source.lines().size(); i++) {
            ...
            int later = laterById.getOrDefault(line.id(), 0); laterById.remove(line.id());
            ...
            if (current > 0) { currentLines.add(line(line, current, line.deliveryWeek(), false, priced)); currentTerms.add(first); }
        ...
        if (currentLines.isEmpty() || laterLines.isEmpty()) throw new BusinessRuleException("Beide leveringen moeten minstens één volle doos bevatten");
        ...
        var firstTerms = new SalesSplitPricing(currentTerms, total.orderDiscountPercent(), currentOrderDiscount, currentPct, currentExtra,
                ...
                selection.currentExtraDiscountPct() == null ? firstDefaultGoods : currentNet.subtract(currentOrderDiscount).subtract(currentExtra));
        ...
        if (delta.signum() != 0) warnings.add("Door de gekozen transportkosten of extra korting verandert het totaal exclusief btw met € " + delta.toPlainString() + ".");
        ...
        return new Plan(source, current, later, firstTerms, lastTerms,
                new Preview(token, source.id(), summary(original), summary(firstPriced), summary(laterPriced), List.copyOf(warnings), delta, deltaIncl), selection);
    }
    ...
    private void requireEligible(SalesOrder source) {
        ...
        if (source.status() != QuoteStatus.CONCEPT || source.sentAt() != null || source.viewedAt() != null
                || source.viewCount() != 0 || source.decidedAt() != null || source.portalToken() != null
    ...
    private static SalesOrderLine line(SalesOrderLine source, int quantity, String week, boolean fresh, PricedOrder.Line priced) {
        return new SalesOrderLine(fresh ? null : source.id(), source.productId(), quantity, priced.unitPrice(), source.manualDiscountPct(), week, priced.landedUnitCost());
    }
